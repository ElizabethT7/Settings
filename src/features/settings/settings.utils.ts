...
export const setSettingsState = (
  ...
  if (
    name === "reverse" &&
    !Object.values(changedState).includes(2) &&
    !Object.values(changedState).includes(1)
  ) {
  ...
  if (Object.values(changedState).includes(2)) {
    ...
    for (let key in changedState) {
      changedState[key] =
        changedState[key] > 0 && name !== "reverse" ? changedState[key] - 1 : 0;
    }
    ...
    if (name === "reverse") {
      for (let key in changedState) {
        changedState[key] = changedState[key] > 0 ? 0 : 2;
      }
    }
  }
...
export const settingsControl = [
  ...
  { name: "reverse", label: "Поменять настройки" },
];
